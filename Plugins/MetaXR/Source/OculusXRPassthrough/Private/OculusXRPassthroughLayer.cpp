...
namespace XRPassthrough
{
	...
	void FPassthroughLayer::ClearPassthroughPokeActors()
	{
		if (PassthroughPokeActorMap)
		{
			...
			{
				TMap<FString, FPassthroughPokeActor> ActorArray = *PassthroughPokeActorMap.Get();
				ExecuteOnGameThread(TEXT("Clear PassthroughPokeActor array."), [ActorArray]() {
					UWorld* World = GetWorld();
					if (!World)
					{
						UE_LOG(LogOculusXRPassthrough, Warning, TEXT("Couldn't retrieve World. Passthrough Pokeahole actors will not be destroyed."));
					...
					for (const TPair<FString, FPassthroughPokeActor>& Entry : ActorArray)
					{
						World->DestroyActor(Entry.Value.PokeAHoleActor);
					}
				});
